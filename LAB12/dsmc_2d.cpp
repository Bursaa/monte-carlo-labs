...
void DSMC_2D::evolution(double time_max, int nit, const char *katalog_name)
{
	int i, it = 0;
	time_t czas1, czas2;
	double czas; // czas trwania obliczen na komputerze [s]
	string nazwa_nptv, nazwa_xy, nazwa_hist, katalog;
	...
	while (time_sum < time_max || it < nit)
	{
		...
		if (it % 2 == 0 || it < 10)
		{
			nazwa_nptv = katalog + string("/nptv_") + to_string(it) + string(".dat");
			nazwa_xy = katalog + string("/xy_") + to_string(it) + string(".dat");
			nazwa_hist = katalog + string("/hist_") + to_string(it) + string(".dat");

			write_nptv(nazwa_nptv.c_str(), 1);
			FILE *fp;
			fp = fopen(nazwa_xy.c_str(), "w");
			for (i = 0; i < ntot; i += 100)
			{
				fprintf(fp, "%15.7E   %15.7E   %15.7E   %15.7E\n", par[i].x, par[i].y, par[i].vx, par[i].vy);
			}
			fflush(fp);
			fclose(fp);
...
inline void DSMC_2D::sort()
{
	...
	for (i = 0; i < ntot; i++)
	{
		...
		y = par[i].y;
		if (x < xmin)
			par[i].x = xmin + delta_x * 1.0E-3;
		if (x > xmax)
			par[i].x = xmax - delta_x * 1.0E-3;
		if (y < ymin)
			par[i].y = ymin + delta_y * 1.0E-3;
		if (y > ymax)
			par[i].y = ymax - delta_y * 1.0E-3;
	}
...
void DSMC_2D::hist_velocity_all(const char *text, double v_multi, int nhist)
{
	...
	for (i = 0; i < nhist; i++)
	{
		...
		fprintf(fp, "%12.4E ", v);
		for (ic = 0; ic < n_mix; ic++)
			fprintf(fp, "%12.4E ", hist_v_num[ic][i]);
		for (ic = 0; ic < n_mix; ic++)
			fprintf(fp, "%12.4E ", hist_v_teo[ic][i]);
		fprintf(fp, "\n");
	}
...
inline void DSMC_2D::temperature_cell()
{
	for (int ix = 0; ix < nx; ix++)
	{
		for (int iy = 0; iy < ny; iy++)
		{
			...
			if (indx0[m][0] > 0)
				temp_cell[ix][iy] = temp_cell[ix][iy] / kb / indx0[m][0];
		}
	}
	return;
}
...
void DSMC_2D::read(const char *text)
{
	...
	else
	{
		...
		printf("n_mix= %d\n", n_mix);
		for (int i = 0; i < n_mix; i++)
			printf("nc, mc, rc = %d  %10.2g %10.2g \n", nc[i], mc[i], rc[i]);

		printf("nodes= %d\n", nodes);
		if (nodes > 0)
			for (int i = 0; i < nodes; i++)
				printf("(x%d,y%d)=  %10.2g %10.2g \n", i, i, edge[i][0], edge[i][1]);
	}
	fclose(fp_in);
	return;
}
